...
RSpec.describe PhoneNumberService, type: :service do
  describe "#create" do
    context "when params are correct but user is missing" do
      subject { described_class.new.create(phone_number_params: { number: "+49 015 2052 90170" }) }
      it "returns User must exist" do
        _phone_number_object, errors = subject
    ...
    context "when user exists" do
      ...
      subject { described_class.new.create(phone_number_params: { number: "+49 015 2052 90170", user_id: user.id }) }
      it "returns phone_number_object" do
        phone_number_object, _errors = subject
    ...
    context "when phone number is not valid" do
      context "with user exists" do
        let(:user) { create(:user) }
        subject { described_class.new.create(phone_number_params: { number: "+201002216385", user_id: user.id }) }
        it "does not create a phone number" do
          expect { subject }.not_to change { PhoneNumber.count }.from(0)
          _phone_number_object, errors = subject
      ...
      context "without user exists" do
        subject { described_class.new.create(phone_number_params: { number: "+201002216385", user_id: nil }) }
        it "does not create a phone number" do
          expect { subject }.not_to change { PhoneNumber.count }.from(0)
          _phone_number_object, errors = subject
